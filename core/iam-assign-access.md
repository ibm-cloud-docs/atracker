...
Access to {{site.data.keyword.atracker_full}} configuration resources for users and applications in your account is controlled by {{site.data.keyword.iamlong}} (IAM). Every user or application that accesses the {{site.data.keyword.atracker_full_notm}} service in your account must be assigned an access policy with an IAM role. Review the following roles, actions, and more to help determine the best way to assign access to {{site.data.keyword.atracker_full_notm}}.
{: shortdesc}

You can assign access to {{site.data.keyword.atracker_full_notm}} by using any of the following methods:
...
## Assigning access to {{site.data.keyword.atracker_full_notm}} in the console
{: #assign-access-ui}
{: ui}

There are two common ways to assign access to {{site.data.keyword.atracker_full_notm}} in the console:
...
## Assigning access to {{site.data.keyword.atracker_full_notm}} in the CLI
{: #assign-access-cli}
{: cli}

For step-by-step instructions for assigning, removing, and reviewing access, see [Assigning access ro resources by using the CLI](/docs/account?topic=account-assign-access-resources&interface=cli#access-resources-cli). The following example shows a command for assigning the `Viewer` role for {{site.data.keyword.atracker_full_notm}}:

Use `atracker` for the service name. Also, use quotations around role names that are more than one word.
{: tip}
...
## Assigning access to {{site.data.keyword.atracker_full_notm}} by using the API
{: #assign-access-api}
{: api}
...
| Role name | Role CRN |
|---------------|-----------------|
| Viewer                 | `crn:v1:bluemix:public:atracker::::serviceRole:Viewer`        |
| Operator               | `crn:v1:bluemix:public:atracker::::serviceRole:Operator`      |
| Editor                 | `crn:v1:bluemix:public:atracker::::serviceRole:Editor`        |
| Administrator          | `crn:v1:bluemix:public:atracker::::serviceRole:Administrator` |
{: caption="Table 2. Role ID values for API use" caption-side="bottom"}

The following example is for assigning the `Viewer` role for {{site.data.keyword.atracker_full_notm}}:

Use `atracker` for the service name, and refer to the Role ID values table to ensure that you're using the correct value for the CRN.
{: tip}
...
PolicyRole policyRoles = new PolicyRole.Builder()
      .roleId("crn:v1:bluemix:public:atracker::::serviceRole:Viewer")
      .build();
...
policyRoles := &iampolicymanagementv1.PolicyRole{
  RoleID: core.StringPtr("crn:v1:bluemix:public:atracker::::serviceRole:Viewer"),
}
...
## Assigning access to {{site.data.keyword.atracker_full_notm}} by using Terraform
{: #assign-access-terraform}
{: terraform}

The following example is for assigning the `Viewer` role for {{site.data.keyword.atracker_full_notm}}:

Use `atracker` for the service name.
{: note}
