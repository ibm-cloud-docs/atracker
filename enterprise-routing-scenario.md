...
You can configure your enterprise child accounts to route all or a subset of activity tracker events to an enterprise parent account. The child accounts contain Cloud resources that generate activity tracker events. The parent account owns the enterprise and has {{site.data.keyword.logs_full_notm}} instances that will contain the activity tracker events from the child accounts.

The enterprise-managed routing configurations, from child accounts to parent account, can be locked by restricting the {{site.data.keyword.atracker_short}} enterprise actions with IAM Action Control. The child accounts can continue to manage their own account-managed {{site.data.keyword.atracker_short}} configurations, while the enterprise-managed configurations are restricted.
...
![A diagram that shows a sample {{site.data.keyword.atracker_full_notm}} enterprise configuration.](/images/atracker-enterprise-routing.svg "{{site.data.keyword.atracker_full_notm}} enterprise configuration."){: caption="{{site.data.keyword.atracker_full_notm}} enterprise configuration" caption-side="bottom"}
...
Follow [Provisioning an {{site.data.keyword.logs_full_notm}} instance by using Terraform](/docs/cloud-logs?topic=cloud-logs-terraform-setup) to create the instance in the enterprise parent account. The instance will not receive activity tracker events until the child accounts are configured to send their events to the instance.
...
This step ensures {{site.data.keyword.atracker_full}} is authorized to route child A and child B account activity tracker events to parent account.
...
Next, you need to create {{site.data.keyword.atracker_full_notm}} enterprise targets in each child account, the target destination is the parent account's {{site.data.keyword.logs_full_notm}} instance. In this example, there are only two child accounts, but there is no limitation to the number of child accounts that can be targeted to a single parent account.
...
resource "ibm_atracker_target" "atracker_target" {
  cloudlogs_endpoint {
    target_crn = "<INPUT_PARENT_LOGS_CRN>"
  }
  name = "child-a-target"
...
This step creates {{site.data.keyword.atracker_short}} enterprise targets pointing to the parent {{site.data.keyword.logs_full_notm}} instance. Routing is not yet enabled.

## Step 4 - Create enterprise routes in the child accounts
{: #step-create-child-enterprise-routes}
{: terraform}

Create {{site.data.keyword.atracker_full_notm}} enterprise routes in each child account. You can route all activity tracker events to the parent or a subset of activity tracker events to the parent. In this example, child A and child B accounts will route all activity tracker events to the parent.
...
resource "ibm_atracker_route" "atracker_enterprise_route" {
  lifecycle {
  ...
  # Route all activity tracker events to the parent.
  rules {
...
This step creates the {{site.data.keyword.atracker_short}} enterprise routes. Activity tracker events will be routed to the parent {{site.data.keyword.logs_full_notm}} instance.
...
Now that the enterprise-managed routing is created, you must restrict the usage of {{site.data.keyword.atracker_short}} enterprise actions so that the enterprise-managed routing cannot be modified by any entity. In this example, there is only one version of the action control template and it is assigned to each individual Account instead of the entire Enterprise or an Account Group.
...
resource "ibm_iam_action_control_template" "atracker_enterprise_action_control_template" {
  name = "ATracker Enterprise Routing"
  description = "Restrict ATracker enterprise-managed routing"
  action_control {
    actions = [
      "atracker.enterprise-target.create",
      "atracker.enterprise-target.update",
      "atracker.enterprise-target.delete",
      "atracker.enterprise-route.create",
      "atracker.enterprise-route.update",
      "atracker.enterprise-route.delete"
    ]
...
resource "ibm_iam_action_control_assignment" "atracker_enterprise_action_control_assignment" {
  for_each = toset(local.child_accounts)
  target = {
    type = "Account"
    id   = each.key
  ...
  templates {
    id      = ibm_iam_action_control_template.atracker_enterprise_action_control_template.template_id
    version = ibm_iam_action_control_template.atracker_enterprise_action_control_template.version
  }
}
```
{: codeblock}

This step ensures {{site.data.keyword.atracker_short}} enterprise actions are restricted from further use, locking the enterprise-managed routing.
