...
# Creating a S2S authorization to grant access to the {{site.data.keyword.appconfig_notm}} service
{: #iam-service-auth-appconfig}

Use {{site.data.keyword.iamlong}} (IAM) to create an authorization that grants {{site.data.keyword.atracker_full_notm}} access to the {{site.data.keyword.appconfig_notm}} service.
{: shortdesc}

You must configure the service to service (S2S) authorization in the {{site.data.keyword.cloud_notm}} account where the {{site.data.keyword.appconfig_notm}} instance is located.
{: important}

## Before you begin
{: #iam-service-auth-appconfig-prereqs}
...
- The autorization that you define for the {{site.data.keyword.atracker_full_notm}} service requires that you have `Administrator` role for the {{site.data.keyword.appconfig_notm}} target instance.

- Make sure that you are defining the authorization in the account where the {{site.data.keyword.appconfig_notm}} instance is located.
...
{: #iam-service-auth-appconfig-roles}
...
{: #iam-service-auth-appconfig-create-ui}
{: ui}
...
3. Select a source account.
    ...
    If {{site.data.keyword.atracker_full_notm}} and the {{site.data.keyword.appconfig_notm}} instance are in the same account where you are defining the authorization, select **This account**.

    If {{site.data.keyword.atracker_full_notm}} and the {{site.data.keyword.appconfig_notm}} instance are in different accounts, select **Other account**. Then, enter the account ID of the source account, that is, the account where {{site.data.keyword.atracker_full_notm}} is to be configured to send data to an {{site.data.keyword.appconfig_notm}} instance.

4. Select `Activity Tracker Event Routing` as the source service. Then, set the scope of the access to **All resources**.

5. Select **App Configuration** as the target service. Then, set the scope of the access.
...
6. In the *Service Access* section, select **Configuration Update Reporter** to assign {{site.data.keyword.atracker_full_notm}} access to the the {{site.data.keyword.appconfig_notm}} instance.
...
{: #iam-service-auth-appconfig-create-cli}
{: cli}
...
- `APPCONFIG_SERVICE_INSTANCE`: ID of the {{site.data.keyword.appconfig_notm}} instance.
...
{: #iam-service-auth-appconfig-create-cli-cross}
{: cli}
...
```sh
ibmcloud iam authorization-policy-create atracker apprapp "ConfigurationUpdateReporter" [--target-service-instance-id <APPCONFIG_SERVICE_INSTANCE> [--source-service-account SOURCE_SERVICE_ACCOUNT_GUID ]
```
{: codeblock}

Where you can set the following parameters to grant access to a single bucket:
- `APPCONFIG_SERVICE_INSTANCE`: CRN of the {{site.data.keyword.appconfig_notm}} instance.
...
{: #iam-service-auth-appconfig-create-terra}
{: terraform}
...
1. Create an authorization policy between services by using the `ibm_iam_authorization_policy` resource argument in your `main.tf` file.
    ...
    resource "ibm_iam_authorization_policy" "policy" {
      ...
      roles                       = ["ConfigurationUpdateReporter"]
    }
    ```
    {: codeblock}
...
{: #iam-service-auth-appconfig-create-api}
{: api}
